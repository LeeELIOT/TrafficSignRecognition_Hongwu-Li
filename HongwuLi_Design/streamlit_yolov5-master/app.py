...
if prediction_mode == 'Single image':
    ...
    if uploaded_file is not None:
        ...
        for bbox_data in result_copy:
            ...
            img_draw = cv2.rectangle(img_draw, 
                                    p0, p1, 
                                    rgb_colors[label], 2)
            cv2img = cv2.cvtColor(img_draw, cv2.COLOR_BGR2RGB)
            pilimg = Image.fromarray(cv2img)
            draw = ImageDraw.Draw(pilimg)  # 图片上打印
            font = ImageFont.truetype("simhei.ttf", 20, encoding="utf-8")
            draw.text((int(xmin),int(ymin)-20), CLASSES[label], (rgb_colors[label][2],rgb_colors[label][1],rgb_colors[label][0]), font=font) 
            img_draw = cv2.cvtColor(np.array(pilimg), cv2.COLOR_RGB2BGR) 
    ...
    if st.button ('Detect Objects'):
        ...
        while True:
            ...
            #также скопируем изображение, чтобы не изменить аргумент кэшируемой 
            # функции get_preds
            img_draw = frame.copy().astype(np.uint8)
            # нарисуем боксы для всех найденных целевых объектов
            for bbox_data in result_copy:
                xmin, ymin, xmax, ymax, _, label = bbox_data
                p0, p1, label = (int(xmin), int(ymin)), (int(xmax), int(ymax)), int(label)
                img_draw = cv2.rectangle(img_draw, 
                                        p0, p1, 
                                        rgb_colors[label], 2)
                #cv2.putText(img2, label, (xmin, ymin), cv2.FONT_HERSHEY_PLAIN,
                            #1.0, rgb_colors[label], thickness=1)
                cv2img = cv2.cvtColor(img_draw, cv2.COLOR_BGR2RGB)
                pilimg = Image.fromarray(cv2img)
                draw = ImageDraw.Draw(pilimg)  # 图片上打印
                font = ImageFont.truetype("simhei.ttf", 20, encoding="utf-8")
