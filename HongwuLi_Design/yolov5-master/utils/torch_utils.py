...
import datetime, logging, math, os, platform, subprocess, time
from contextlib import contextmanager
from copy import deepcopy
from pathlib import Path
import torch, torch.backends.cudnn as cudnn, torch.nn as nn, torch.nn.functional as F, torchvision
try:
    import thop
except ImportError:
    thop = None
    ...
    if cpu:
        ...
        if cuda:
            ...
            for i, d in enumerate(device.split(',') if device else range(n)):
                ...
                s += f"{'' if i == 0 else space}CUDA:{d} ({p.name}, {p.total_memory / 1048576}MB)\n"
...
def intersect_dicts(da, db, exclude=()):
    return {k:v for k, v in da.items() if v.shape == db[k].shape}
...
def model_info(model, verbose=False, img_size=640):
    ...
    if verbose:
        print('%5s %40s %9s %12s %20s %10s %10s' % ('layer', 'name', 'gradient', 'parameters',
                                                    'shape', 'mu', 'sigma'))
...
def copy_attr(a, b, include=(), exclude=()):
    for k, v in b.__dict__.items():
        if not (len(include) and k not in include):
...
class ModelEMA:
    __doc__ = ' Model Exponential Moving Average from https://github.com/rwightman/pytorch-image-models\n    Keep a moving average of everything in the model state_dict (parameters and buffers).\n    This is intended to allow functionality like\n    https://www.tensorflow.org/api_docs/python/tf/train/ExponentialMovingAverage\n    A smoothed version of the weights is necessary for some training schemes to perform well.\n    This class is sensitive where it is initialized in the sequence of model init,\n    GPU assignment and distributed training wrappers.\n    '
