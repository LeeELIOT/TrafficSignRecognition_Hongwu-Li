...
import glob, hashlib, logging, math, os, random, shutil, time
...
class InfiniteDataLoader(torch.utils.data.dataloader.DataLoader):
    __doc__ = ' Dataloader that reuses workers\n    Uses same syntax as vanilla DataLoader\n    '
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False, cache_images=False, single_cls=False, stride=32, pad=0.0, prefix=''):
        ...
        if cache_images:
            ...
            for i, x in pbar:
                ...
                pbar.desc = f"{prefix}Caching images ({gb / 1000000000.0:.1f}GB)"
        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        if not l.shape[1] == 5:
                            raise AssertionError('labels require 5 columns each')
                        else:
            ...
            pbar.desc = f"{prefix}Scanning '{path.parent / path.stem}' images and labels... {nf} found, {nm} missing, {ne} empty, {nc} corrupted"
    ...
    def collate_fn(batch):
        ...
        return (
         torch.stack(img, 0), torch.cat(label, 0), path, shapes)
...
def load_image(self, index):
    ...
    if img is None:
        ...
        return (
         self.imgs[index], self.img_hw0[index], self.img_hw[index])
            ...
            if i == 1:
                ...
                if i == 2:
                    ...
                    if i == 3:
                        ...
                        if i == 4:
                            c = (
                             s + w0, s + hp, s + w0 + w, s + hp + h)
                        else:
                            if i == 5:
                                c = (
                                 s + w0 - w, s + h0, s + w0, s + h0 + h)
                            else:
                                if i == 6:
                                    c = (
                                     s + w0 - wp - w, s + h0, s + w0 - wp, s + h0 + h)
                                else:
                                    if i == 7:
                                        c = (
                                         s - w, s + h0 - h, s, s + h0)
...
def random_perspective(img, targets=(), segments=(), degrees=10, translate=0.1, scale=0.1, shear=10, perspective=0.0, border=(0, 0)):
    ...
    if border[0] != 0 or border[1] != 0 or (M != np.eye(3)).any():
        if perspective:
            img = cv2.warpPerspective(img, M, dsize=(width, height), borderValue=(114,
                                                                                  114,
                                                                                  114))
        else:
            img = cv2.warpAffine(img, (M[:2]), dsize=(width, height), borderValue=(114,
                                                                                   114,
                                                                                   114))
    n = len(targets)
    if n:
