...
import cv2, matplotlib, matplotlib.pyplot as plt, numpy as np, pandas as pd, seaborn as sns, torch, yaml
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
    if isinstance(images, torch.Tensor):
        ...
        for i, img in enumerate(images):
            ...
            cv2.rectangle(mosaic, (block_x, block_y), (block_x + w, block_y + h), (255,
                                                                                   255,
                                                                                   255), thickness=3)
...
def plot_study_txt(path='', x=None):
    ...
    for f in sorted(Path(path).glob('study*.txt')):
        ...
        j = y[3].argmax() + 1
        ax2.plot((y[6, 1:j]), (y[3, 1:j] * 100.0), '.-', linewidth=2, markersize=8, label=(f.stem.replace('study_coco_', '').replace('yolo', 'YOLO')))

    ax2.plot((1000.0 / np.array([209, 140, 97, 58, 35, 18])), [34.6, 40.5, 43.0, 47.5, 49.7, 51.5], 'k.-',
      linewidth=2, markersize=8, alpha=0.25, label='EfficientDet')
    ax2.grid(alpha=0.2)
...
def plot_results_overlay(start=0, stop=0):
    s = [
     'train', 'train', 'train', 'Precision', 'mAP@0.5', 'val', 'val', 'val', 'Recall', 'mAP@0.5:0.95']
